...
/* This function uses conditional statements to test whether arguments have been 
  ...
  The following variation of the processOrder function has the same behavior as t
  he first: */
...
/* The limitation of these solutions is that when one of the optional arguments 
  has a value of 0, the function incorrectly treats that argument as if it has 
  been omitted. For example:

processOrder(100, 2, 0.1, 0, 0);      // 227.7 -- incorrect result!

  This happens because 0 is a falsy value in JavaScript, so any arguments with a 
  value of 0 get reassigned a default value. One way to prevent this from 
  happening is to explicitly test whether the input arguments have a value of 
  undefined, instead of relying on them being falsy — e.g., 
  (quantity === undefined). (Note that when the value passed for discount is 0, 
    the function works as expected because the default value of discount is also 
    0.) */