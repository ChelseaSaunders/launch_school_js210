/* With arrays, you can access the first element's value with [0], but how do 
  you access the last value? Write a function named lastInArray that returns the 
...
/* Create a function that returns the contents of the array it receives as an 
  ...
  array to the beginning, and pushes each element's value to a new result array. 
  Be sure to start your loop with the element whose index is one less than the 
...
function arrayToString(array) {
  let arrayValuesAsStrings = ''

  for (let index = 0; index < array.length; index += 1) {
    arrayValuesAsStrings += String(array[index]);
  }

  return arrayValuesAsStrings;
}
