...
// 5. Declare a variable named start and set it to incrimenter.
let start = incrimenter;

// 6. Declare variables named end and difference. Leave them undefined.
let end;
let difference;

/* 7 In JavaScript, we can use a shorthand operator to add or subtract values to
  and from a variable. If we want to add one to our incrementer variable, we can 
  write incrementer = incrementer + 1. However, JavaScript lets us shorten this 
  with the self-addition operator, +=, e.g., incrementer += 1. (All we did was 
  remove the second instance of incrementer, and then recombined the = and + as 
  +=.) Analogous arithmetic assignment operators are available for the other 
  basic arithmetic operations: subtraction, multiplication, and division.
